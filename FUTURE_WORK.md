...
## `[[`/`]]` are separate lexer tokens with no context-aware disambiguation, breaking naively-written nested array literals

**Status:** not started, deliberately deferred; workable today via a space (`[ [1, 2], [3, 4] ]` instead of `[[1, 2], [3, 4]]`).

**Problem:** `annium.l:177-178` defines `OPEN_SQUARE_DBL_BRACKET "[["` / `CLOSE_SQUARE_DBL_BRACKET "]]"` as their own tokens (used by `annium.y:1081-1082` to force an array-literal reading regardless of element count — see the entry above). Flex tokenizes by longest match with no grammar-context awareness, so *any* two `[` (or `]`) characters typed adjacently anywhere in an expression collapse into one `[[`/`]]` token, whether or not that's where the writer intended a double-bracket array literal. Concretely, `[[1, 2], [3, 4], [5, 6]]` (an array-of-arrays literal, written the naive way) fails to parse: the lexer reads the leading `[[` as one token, so the parser expects the whole construct to close with `]]`, and errors out at the first inner `]` ("unexpected `]`, expecting `]]` or `,`"). This is the same family of problem as pre-C++11 `>>` closing nested template argument lists.

**Consequence:** nested fixed-array-of-fixed-array literals need a space between adjacent brackets to parse (`[ [1, 2], [3, 4] ]`, and equally at the closing end if the innermost element also ends in `]`). Not documented anywhere a reader would find it before hitting the confusing error. Worked around in `array_operations.ann`'s `ca2` test with explicit spacing; see the comment there.

**Proposed direction:** not decided. Options include: (a) just document the spacing requirement (in `ANNIUM_SYNTAX.md`) and leave the tokens as-is — cheapest, but the error message stays confusing for anyone who hits it cold; (b) make the lexer/parser cooperate to split a `]]`/`[[` token back into two single-bracket tokens when the parser's current state can't accept the double form (mirrors how C++11 resolved `>>` in template contexts) — more robust but a real parser-integration change; (c) reconsider whether `OPEN_SQUARE_DBL_BRACKET`/`CLOSE_SQUARE_DBL_BRACKET` need to exist as distinct lexer tokens at all, versus e.g. having the grammar accept `OPEN_SQUARE_BRACKET OPEN_SQUARE_BRACKET` (two adjacent single-bracket tokens) for the same forced-array-literal production, if there's no other reason double-brackets need to be a single lexeme.

**Why deferred:** found via the same `fixed_array_get_pattern` test-writing session as the entry above; a lexer/grammar change is out of scope for that task and needs its own design decision, not a drive-by fix.
