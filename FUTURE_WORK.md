...
**2. Struct fields and array elements are still not referenceable (tuple elements now are).** `base_expression_visitor::try_take_reference` (`ast/base_expression_visitor.cpp`) only fires for a plain local variable/parameter. A tuple element (`t.0`/`t[i]`, for a tuple with more than one runtime field) is referenceable too now — see `IMPLEMENTATION_NOTES.md`'s "Referencing a tuple element" section for the mechanism (`blob_reference` generalized to point at a raw scalar, plus `tuple_get_pattern`'s ref-mode wiring). A struct field (`obj.field`) and an array element (`arr[i]`, packed or boxed) could in principle be wired up the same way — the representation problem that used to block both is gone (the same generalized `blob_reference` covers them), only the annium-side `try_match`/`apply` wiring (mirroring `tuple_get_pattern`'s change) remains.
