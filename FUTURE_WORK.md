...
## Extend generic numeric arithmetic (`__plus`/`__minus`) to `__mul`/`__div`

**Status:** not started, deliberately deferred (`__plus` and `__minus` are done — see `IMPLEMENTATION_NOTES.md`'s "Generic numeric arithmetic" section).

**Problem:** `__mul`/`__div` don't exist as callable operators at all.

**Proposed direction:** the reusable pieces are already in `entities/literals/numeric_promotion.hpp` (`strongest_numeric_type`, `widen_for_literal_fit`, the shared value-conversion helpers, `add_numeric`/`subtract_numeric` as the template to follow for `multiply_numeric`/`divide_numeric`). Each new operator needs: (1) a `numeric_literal_<op>_pattern` mirroring `numeric_literal_plus_pattern`/`numeric_literal_minus_pattern`'s try_match/apply shape (multiplication is commutative like addition; division, like subtraction, must preserve operand order), (2) a type-erased runtime extern mirroring `annium_operator_plus_numeric`/`annium_operator_minus_numeric` (`numeric_builtin_eid_of` + a per-result-type switch — note `numetron::decimal` has no binary `operator-`, only `operator+`/unary `operator-()`, which is why `subtract_numeric` computes `lhs + (-rhs)`; check what binary operators `numetron::decimal`/`numetron::integer` actually expose before assuming `*`/`/` exist directly), (3) the corresponding `builtin_eid` slot and registration in `annium_environment.cpp`. Division additionally needs a decision this codebase hasn't made yet: what `strongest_numeric_type` should mean for two *integral* operands (does `i32/i32` truncate like C, or promote to `decimal`?) — plausibly a different join rule than addition/subtraction/multiplication.

**Why deferred:** explicitly out of scope for the `__plus`/`__minus` task this grew out of; the user asked for the mechanism to be *designed* reusably but only `__plus`/`__minus` to be *implemented* so far.
