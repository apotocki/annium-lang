...
## Decide what `decimal` division should mean

**Status:** not started, deliberately deferred (`__plus`, `__minus`, `__multiply`, and `__divide` for every *non*-`decimal` numeric type are done — see `IMPLEMENTATION_NOTES.md`'s "Generic numeric arithmetic" section).

**Problem:** `numeric_literal_div_pattern` rejects `decimal` on either side of `/` outright (a compile error, `type_mismatch_error`), rather than joining into it the way `+`/`-`/`*` do. This isn't an oversight — division for an arbitrary-precision decimal type doesn't have an obvious answer the way addition/subtraction/multiplication do (the result of `1 / 3` isn't exactly representable in any finite decimal significand/exponent pair), so it needs an actual design decision rather than a drive-by implementation: does `decimal / decimal` round to some fixed precision, truncate, throw on non-terminating results, return a different type (e.g. always promote to a fixed-width float), or something else?

**Proposed direction:** not decided. Once a rule is picked, the shape to wire it in is exactly `add_numeric`/`subtract_numeric`/`multiply_numeric`/`divide_numeric`'s existing pattern (`numeric_promotion.hpp`/`.cpp`) — add a `case builtin_eid::decimal:` to `divide_numeric`'s switch, and remove the `lhs_type == decimal || rhs_type == decimal` (and the three `result_type == decimal`) guards from `numeric_literal_div_pattern::try_match`.

**Why deferred:** explicitly out of scope for the `__plus`/`__minus`/`__multiply`/`__divide` task this grew out of — the user asked for `decimal` division to be left undefined for now, pending a separate conversation about what it should mean.
