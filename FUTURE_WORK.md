...
**Status:** v1 (reference to a plain local variable/parameter) implemented and working — see `IMPLEMENTATION_NOTES.md`'s `ref(T)` section and `BUGFIXES.md`'s `reference_blob_result` entry for the mechanism and the bug that had to be fixed to make it work. The items below were identified in review right after v1 landed; item 2 is now partly done (tuple and array elements, struct fields still open), the rest are not started.
...
A tuple-element reference (`ref(of: E)` from `t.0`, see item 2) has a narrower version of this same gap that a whole-variable `ref(T)` does *not*: reassigning the whole tuple variable (`t = <new tuple>;`) while an element reference is still outstanding invalidates it immediately, even though `t` itself is still validly in scope — heap-backed tuples free the old buffer out from under the reference, inplace tuples silently overwrite the bytes it points at. A whole-variable `ref(T)` observes the slot's *current* value and is unaffected by this. No mitigation designed; same category of gap as the rest of this item, just reachable one step earlier.

**2. Struct fields are still not referenceable (tuple elements and array elements now are).** `base_expression_visitor::try_take_reference` (`ast/base_expression_visitor.cpp`) only fires for a plain local variable/parameter. A tuple element (`t.0`/`t[i]`) and an array element (`arr[i]`, const or runtime index, packed or boxed) are referenceable too now — see `IMPLEMENTATION_NOTES.md`'s "Referencing a tuple element" and "Array elements" sections for the mechanism (`blob_reference` generalized to point at a raw scalar, plus `tuple_get_pattern`'s/`fixed_array_get_pattern`'s shared ref-mode wiring). A struct field (`obj.field`) could in principle be wired up the same way — the representation problem that used to block all three is gone, only the annium-side `try_match`/`apply` wiring (mirroring the tuple/array change) remains, presumably in `struct_get_pattern.cpp`.

**3. `functional_variable` (module/global-scope variables, as opposed to locals/parameters) never gets the `try_take_reference` treatment.** Passing one where a `ref(of: T)` parameter is expected falls through to the ordinary `apply_cast` path, finds no `implicit_cast` for `T -> ref(T)`, and fails to compile with a plain type-mismatch error. Not dangerous (fails loudly, not silently), but unexplored — not established whether extending `try_take_reference` to this case is even sound, given a global's lifetime is different from a stack slot's.

**4. Nested references (`ref(of: ref(of: T))`) are untested.** `smart_blob::operator=`'s write-through branch (`invocation.hpp`) already has explicit handling for "the right-hand side is itself a reference" (`if (is_ref(rhs->type)) { ... }`), suggesting the original (pre-`ref(T)`) author anticipated some reassignment-through-nested-references semantics, but nothing in the current `ref(T)` design has exercised or specified what `set(self: rr, value: ...)` should do when `rr`'s own type is `ref(of: ref(of: i32))` — rebind `rr`'s own target, or write through two levels.

**5. "Forwarding" a reference into another `ref(T)`-taking function works, but by accident, not by design.** `foo($x)` from inside a function that itself received `$x: ref(of: i32)`, where `foo` also wants `ref(of: i32)`, compiles correctly today — but only because `try_take_reference`'s exact-type-match check (`vartype == of_type`) fails (`$x`'s own type is already `ref(of: i32)`, not `i32`) and falls through to plain `apply_cast`, which sees the types already match and passes the reference through as an ordinary copy (correctly preserving aliasing, since copying a `blob_reference` doesn't change what it points at). This happens to be correct but was never a deliberate case in the design or covered by a test.

**Why deferred:** surfaced in a design-review conversation immediately after `ref(T)` v1 shipped; none of the five are blocking anything working today, and each (especially #1) needs its own real design decision rather than a drive-by fix.
