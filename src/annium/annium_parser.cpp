...
class syntax_expression_resource : public code_resource
{
protected:
...
public:
    inline file_resource(environment& e, fs::path p) noexcept
        : syntax_expression_resource{ e }
        , path_{ std::move(p) }
    {}
...
{
public:
    inline explicit string_resource(environment& e, string_view s)
        : syntax_expression_resource{ e, std::string(s) }
    {}
...
shared_ptr<code_resource> parser_context::get_resource() const noexcept
{
    return resource_;
}
...
void parser_context::append_error(resource_location const& loc_begin, std::string errmsg)
{
...
{
    return environment_.push_back_expression(resource_->expressions, std::move(e));
}
...
std::expected<statement_span, std::string> parser_context::parse(fs::path const& f, fs::path const* base_path)
{
    ...
    try {
        code = environment_.get_file_content(f, base_path);
    } catch (std::exception const& e) {
        return std::unexpected(e.what());
    }
    resource_ = make_shared<file_resource>(environment_, f);
...
{
    resource_ = make_shared<string_resource>(environment_, code);
    return parse(code);
}
