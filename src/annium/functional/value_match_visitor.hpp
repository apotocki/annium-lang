...
struct value_match_visitor : static_visitor<std::expected<entity_identifier, error_storage>>
{
    ...
    syntax_expression const& cexpr;
    functional_binding& binding; // to forward it to signature_matcher_visitor
    bool matching_type;

    value_match_visitor(fn_compiler_context& caller, fn_compiler_context& callee, semantic::expression_list_t&, syntax_expression const& ce, functional_binding& b, bool is_type_matching = false) noexcept;
