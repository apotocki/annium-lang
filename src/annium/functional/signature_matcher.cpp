...
signature_matcher_visitor::result_type signature_matcher_visitor::operator()(function_call_t const& sigp) const
{
    ...
    ct_expression_visitor sv{ ctx_, annotated_entity_identifier{ e.get(builtin_eid::qname) } };
    auto qn_ent_id = apply_visitor(sv, sigp.fn_object);
    if (!qn_ent_id) return std::unexpected(std::move(qn_ent_id.error()));
    qname_identifier_entity qname_ent = static_cast<qname_identifier_entity const&>(e.eregistry_get(*qn_ent_id));
    functional const& fn_fl = e.fregistry().resolve(qname_ent.value());
    ...
    entity const& type_ent = e.eregistry_get(ctx_.context_type);
