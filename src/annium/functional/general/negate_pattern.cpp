...
negate_pattern::try_match(fn_compiler_context& ctx, prepared_call const& call, expected_result_t const&) const
{
    ...
    expected_result_t bool_exp{ e.get(builtin_eid::boolean), call.location };
    ...
    pmd->signature.result.emplace(e.get(builtin_eid::boolean), arg->first.is_const_result);
...
negate_pattern::apply(fn_compiler_context& ctx, semantic::expression_list_t& el, functional_match_descriptor& md) const
{
    ...
    e.push_back_expression(el, result.expressions, semantic::invoke_function(e.get(builtin_eid::negate)));
