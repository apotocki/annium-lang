...
public:
    ...
    void operator()(const generic_literal_entity& ent) const override
    {
        ...
        case builtin_eid::integer:
            oss_ << ent.value().as<numetron::integer_view>();
            return;
        case builtin_eid::decimal:
            oss_ << ent.value().as<numetron::decimal_view>();
            return;
...
std::expected<syntax_expression_result, error_storage> to_string_pattern::apply(fn_compiler_context& ctx, semantic::expression_list_t& el, functional_match_descriptor& md) const
{
