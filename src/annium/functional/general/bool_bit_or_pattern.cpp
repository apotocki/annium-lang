...
#include "bool_bit_or_pattern.hpp"
...
// to bool and "match" here too (see BUGFIXES.md), producing a bogus bit_or(bool,bool) candidate
// that competes with numeric_literal_bit_or_pattern instead of losing to it. Fetching with no
...
bool_bit_or_pattern::try_match(fn_compiler_context& ctx, prepared_call const& call, expected_result_t const&) const
{
...
bool_bit_or_pattern::apply(fn_compiler_context& ctx, semantic::expression_list_t& el, functional_match_descriptor& md) const
{
    ...
    e.push_back_expression(el, result.expressions, semantic::invoke_function(e.get(builtin_eid::bitor_bool)));
