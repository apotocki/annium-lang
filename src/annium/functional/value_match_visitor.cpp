...
value_match_visitor::value_match_visitor(fn_compiler_context& caller, fn_compiler_context& callee, semantic::expression_list_t& ael, syntax_expression const& ce, functional_binding& b, bool is_type_matching) noexcept
...
value_match_visitor::result_type value_match_visitor::operator()(function_call_t const& fc) const
{
    ...
    entity const& type_ent = get_entity(e, res->value);
...
value_match_visitor::result_type value_match_visitor::operator()(qname_reference const& var) const
{
    ...
    return apply_visitor(make_functional_visitor<result_type>([this, &var](auto eid_or_var) -> result_type
        {
            if constexpr (std::is_same_v<std::decay_t<decltype(eid_or_var)>, entity_identifier>) {
                ...
                auto res = apply_visitor(base_expression_visitor{ caller_ctx, expressions, expected_result_t{ .type = get_entity(callee_ctx.env(), eid_or_var).get_type(), .modifier = value_modifier_t::constexpr_value } }, cexpr);
                if (!res) return std::unexpected(std::move(res.error()));
