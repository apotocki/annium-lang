...
#include "annium/ast/expression_visitor.hpp"
...
{
    functional const& fnl = callee_ctx.env().fregistry_resolve(aqi.value);
...
value_type_match_visitor::result_type value_type_match_visitor::match_type(entity_identifier const& eid, resource_location eidloc) const
{
...
value_type_match_visitor::result_type value_type_match_visitor::operator()(qname_reference const& var) const
{
    ...
    return apply_visitor(make_functional_visitor<result_type>([this, &var](auto eid_or_var) -> result_type
    {
        if constexpr (std::is_same_v<std::decay_t<decltype(eid_or_var)>, entity_identifier>) {
            if (!eid_or_var) { // if not defined
                ...
                // bind variable
                return annium::match_type(caller_ctx, expressions, expr, entity_identifier{}, var.name.location, [this, var](entity_identifier matched_type, bool casted) -> error_storage {
                    if (casted) { --weight; }
...
value_type_match_visitor::result_type value_type_match_visitor::operator()(function_call_t const& fc) const
{
    ...
    return annium::match_type(caller_ctx, expressions, expr, entity_identifier{}, fc.location, [this, &qname_ent, &fc](entity_identifier matched_type, bool casted) -> error_storage {
        if (casted) { --weight; }
        entity const& type_ent = get_entity(caller_ctx.env(), matched_type);
    ...
    //entity const& type_ent = e.eregistry_get(caller_ctx.context_type);
    //entity_signature const* psig = type_ent.signature();
    //if (!psig) {
    ...
    //qname_reference const* pfn_name = get<qname_reference>(&fc.fn_object);
    ...
    //if (!res) return std::unexpected(std::move(res.error()));
    //
    //entity const& type_ent = ctx.env().eregistry_get(ctx.context_type);
    //entity_signature const* psig = type_ent.signature();
    //if (!psig) {
    //    return std::unexpected(make_error<basic_general_error>(fc.location(), "argument mismatch"sv, expr));
    //}
...
value_type_match_visitor::result_type value_type_match_visitor::operator()(placeholder const& ph) const
{
    ...
    //if (!res) return std::unexpected(std::move(res.error()));
    //if (entity_identifier const* peid = get<entity_identifier>(&res->first); peid && *peid == caller_ctx.env().get(builtin_eid::void_)) {
    //    return std::unexpected(make_error<type_mismatch_error>(get_start_location(expr), *peid, caller_ctx.env().get(builtin_eid::any), ph.location));
    //}
    //return res->first;
    ...
    //        if (caller_ctx.context_type == caller_ctx.env().get(builtin_eid::void_)) {
    //            return entity_identifier{}; // ignore void
    //        }
    //        return std::move(v);
    //    } else {
    //        if (v == caller_ctx.env().get(builtin_eid::void_)) {
    //            // expected value of any type instead of typename void
    //            return std::unexpected(make_error<type_mismatch_error>(get_start_location(expr), v, caller_ctx.env().get(builtin_eid::any), ph.location));
    //        }
    //        return v;
    //        //caller_ctx.append_expression(semantic::push_value{ v });
    //        //entity const& e = caller_ctx.env().eregistry_get(v);
