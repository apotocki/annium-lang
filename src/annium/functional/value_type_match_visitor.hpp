...
struct value_type_match_visitor : static_visitor<std::expected<syntax_expression_result, error_storage>>
{
    ...
    syntax_expression const& expr;
    functional_binding& binding; // to forward it to signature_matcher_visitor
    mutable int weight = 0;

    value_type_match_visitor(fn_compiler_context& caller, fn_compiler_context& callee, semantic::expression_list_t&, syntax_expression const&, functional_binding&) noexcept;
        
    result_type operator()(annotated_qname_identifier const& aqi) const;
    
    result_type operator()(qname_reference const& var) const;
...
private:
    result_type match_type(entity_identifier const&, resource_location = {}) const;
