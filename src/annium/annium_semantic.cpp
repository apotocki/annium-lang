...
std::ostream& signatured_entity::print_to(std::ostream& os, environment const& e) const
{
    entity_signature const& sgn = *signature();
    if (sgn.result->entity_id() == e.get(builtin_eid::typename_)) {
        os << "typename "sv;
    }
    return os << e.print(sgn);
...
//vector_type_entity::vector_type_entity(environment& e, entity_identifier et) noexcept
//    : element_type{ et }
//    , sig_{ e.get(builtin_qnid::vector) }
//{
//    set_type(e.get(builtin_eid::typename_));
//    sig_.push_back(e.get(builtin_id::element), field_descriptor{ et, true });
//}
//
...
//std::ostream& vector_type_entity::print_to(std::ostream& os, environment const& e) const
//{
//    return e.eregistry_get(get_type()).print_to(os << '[', e) << ']';
//}
...
class type_mangler_visitor : static_visitor<qname_identifier>
{
    ...
    inline qname_identifier qnid(identifier id) const { return e_.fregistry().resolve(qname{ id }).id(); }
    inline qname_identifier qnid(string_view sv) const { return qnid(e_.slregistry().resolve(sv)); }

public:
    explicit type_mangler_visitor(environment& e) : e_{ e } {}

    //inline result_type operator()(annium_any_t) const { return qnid("any"sv); }
    //inline result_type operator()(annium_bool_t) const { return qnid("bool"sv); }
    //inline result_type operator()(annium_int_t) const { return qnid("int"sv); }
    //inline result_type operator()(annium_float_t) const { return qnid("float"sv); }
    //inline result_type operator()(annium_decimal_t) const { return qnid("decimal"sv); }
    //inline result_type operator()(annium_string_t) const { return qnid("string"sv); }
        
    inline result_type operator()(annium_object_t const& obj) const { 
        THROW_NOT_IMPLEMENTED_ERROR("type_mangler_visitor annium_object_t");
        //return obj.name();
    }

    inline result_type operator()(annium_fn_type_t const& fn) const
    {
    ...
    inline result_type operator()(annium_array_t const& arr) const
    {
        THROW_NOT_IMPLEMENTED_ERROR();
        /*
    ...
    inline result_type operator()(annium_tuple_t const& tpl) const
    {
        THROW_NOT_IMPLEMENTED_ERROR();
        /*
        ss << '(';
    ...
    inline result_type operator()(annium_bunion_t const& tpl) const
    {
        THROW_NOT_IMPLEMENTED_ERROR();
    }
    inline result_type operator()(annium_union_t const& tpl) const
    {
        THROW_NOT_IMPLEMENTED_ERROR();
        /*
...
void function_signature::setup(fn_compiler_context& ctx, parameter_woa_list_t & params)
{
    ...
    for (auto & parampair : params) {
        annium_type paramtype = apply_visitor(tqvis, parampair.type.value);
        if (!parampair.name.external_name) {
            position_parameters().emplace_back(paramtype);
        } else {
            //GLOBAL_LOG_INFO() << "APPEND param: " << ctx.env().print(parampair.name->id);
...
void function_signature::normilize(fn_compiler_context& ctx)
{
    ...
    //    GLOBAL_LOG_INFO() << "LIST param: " << ctx.env().print(std::get<0>(*k).id);
    //}
    //GLOBAL_LOG_INFO() << "<----------";
    for (auto it = named_parameters().begin(), eit = named_parameters().end();;) {
        ...
        if (std::get<1>(*fit) != std::get<1>(*dupit)) [[unlikely]] {
            ...
            throw exception(ctx.env().print(identifier_redefinition_error{ std::get<0>(*fit), origloc }));
        } // else skip identical
        it = dupit;
    }
...
//void function_signature::build_symbol(environment& e, symbol& s)
//{
//    THROW_NOT_IMPLEMENTED_ERROR("function_signature::build_symbol");
//}

void function_signature::build_mangled_id(environment& e)
{
    type_mangler_visitor vis{ e };

    std::vector<qname_identifier> ps;

    for (annium_type const& postp : position_parameters()) {
        ps.emplace_back(apply_visitor(vis, postp));
    }
    for (auto const& [aname, tp] : named_parameters()) {
        ps.emplace_back(e.qnregistry().resolve(qname{ aname.value }));
        ps.emplace_back(apply_visitor(vis, tp));
    }
    mangled_id = e.piregistry().resolve(ps);
}


annium_type make_union_type(annium_type arg0, annium_type const* parg1)
{
    if (!parg1) return arg0;
    annium_union_t result;
    if (annium_union_t const* pu1 = get<annium_union_t>(parg1); pu1) {
        result = *pu1;
    } else {
        result.append(*parg1);
    }
...
{
    annium_union_t result;
    if (annium_union_t const* pu1 = get<annium_union_t>(&l); pu1) {
        result = *pu1;
    } else if (annium_bunion_t const* pbu1 = get<annium_bunion_t>(&l); pbu1) {
        result.append(pbu1->true_type);
        result.append(pbu1->false_type);
    } else {
        result.append(l);
    }
    if (annium_bunion_t const* pbu2 = get<annium_bunion_t>(&l); pbu2) {
...
{
    annium_union_t& res;
    explicit union_type_remove_visitor(annium_union_t& r) : res{ r } {}
    using basic_type = annium_union_t::basic_type;

    //bool operator()(annium_bool_t const&) const { res.basic_members &= ~(uint16_t)basic_type::bool_e; return true; }
    //bool operator()(annium_int_t const&) const { res.basic_members &= ~(uint16_t)basic_type::int_e; return true; }
    //bool operator()(annium_float_t const&) const { res.basic_members &= ~(uint16_t)basic_type::float_e; return true; }
    //bool operator()(annium_decimal_t const&) const { res.basic_members &= ~(uint16_t)basic_type::decimal_e; return true; }
    //bool operator()(annium_string_t const&) const { res.basic_members &= ~(uint16_t)basic_type::string_e; return true; }
    bool operator()(annium_tuple_t const& t) const
    {
...
{
    annium_union_t result = l;
    if (!apply_visitor(union_type_remove_visitor{ result }, r)) {
        auto it = std::lower_bound(result.other_members.begin(), result.other_members.end(), annium_type{ r });
