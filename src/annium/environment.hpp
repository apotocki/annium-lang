...
{
    BOOST_PP_SEQ_FOR_EACH_I(ANNIUM_PRINT_TYPE_ENUM, _, ANNIUM_BUILTIN_ID_SEQ)
    eof_builtin_id_value
};

enum class builtin_qnid : size_t
{
    BOOST_PP_SEQ_FOR_EACH_I(ANNIUM_PRINT_TYPE_ENUM, _, ANNIUM_BUILTIN_QNAMES_SEQ)
    eof_builtin_qnids_value
};

#undef ANNIUM_PRINT_TYPE_ENUM

// for these types we have builtin_eid == entity_identifier.value
#define ANNIUM_BUILTIN_TYPES_SEQ \
    (boolean)                  \
    (integer)                  \
    (decimal)                  \
    (string)                   \
...
#define ANNIUM_PRINT_BUILTINTYPE_ENUM(r, data, elem) elem,
...
class environment : public enable_shared_from_this<environment>
{
    ...
    using slregistry_t = sonia::lang::string_literal_registry<identifier, small_string>;
    //using qname_registry_t = qname_registry<identifier>;
    using functional_registry_t = sonia::lang::functional_registry<functional>;

    using piregistry_t = sonia::lang::parameterized_identifier_registry<identifier>;
    using eregistry_t = sonia::lang::entity_registry<entity>;
...
public:
    ...
    inline sonia::lang::compiler_task_tracer & task_tracer() noexcept { return task_tracer_; }
    ...
    //variable_entity& new_variable(qname_view, resource_location const&, entity_identifier type, variable_entity::kind);
