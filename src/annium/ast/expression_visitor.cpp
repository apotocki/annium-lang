...
inline expression_visitor::result_type expression_visitor::apply_cast(entity_identifier typeeid, ExprT const& e) const
{
    ...
    if (expected_result.value == env().get(builtin_eid::any)) {
...
{
    ctx.append_expression(semantic::push_value{ d.value });
    return apply_cast(env().get(builtin_eid::decimal), d);
}
...
expression_visitor::result_type expression_visitor::operator()(annotated_string const& s) const
{
    ...
    return apply_cast(env().get(builtin_eid::string), s);
}
...
expression_visitor::result_type expression_visitor::operator()(qname_reference const& var) const
{
 //   THROW_NOT_IMPLEMENTED_ERROR("expression_visitor qname_reference");
    ...
    return apply_visitor(make_functional_visitor<result_type>([this, &var](auto eid_or_var) -> result_type
        {
            if constexpr (std::is_same_v<std::decay_t<decltype(eid_or_var)>, entity_identifier>) {
                if (eid_or_var) {
                    ...
                    entity const& ent = env().eregistry_get(eid_or_var);
    ...
    //    entity const& ent = env().eregistry_get(eid);
    //    return apply_cast(ent.get_type(), var);
    //}
#if 0
        ...
        if (auto varptr = dynamic_cast<variable_entity*>(e.get()); varptr) {
            ...
            if (k == variable_entity::kind::EXTERN) {
                ...
                if (!ctx.env().qnregistry().resolve(varptr->name()).parent().has_prefix(ctx.base_ns())) {
            ...
            if (varptr->is_weak()) {
                ctx.append_expression(semantic::invoke_function{ ctx.env().get_builtin_function(unit::builtin_fn::weak_lock) });
            }
...
//    
//    ctx.append_expression(semantic::invoke_function{ ctx.env().get_builtin_function(unit::builtin_fn::negate) });
//    
//    // "result of negated expression"
//    return apply_cast(annium_bool_t{}, op);
//#endif
//}
...
expression_visitor::result_type expression_visitor::do_assign(binary_expression_t const& op) const
{
    ...
    if (variable_entity const* ve = dynamic_cast<variable_entity const*>(*e); ve) {
        ...
        annium_type const& t = fe->signature().position_parameters().back();
...
{
    entity const& ent = env().eregistry_get(ee.value);
    if (expected_result) {
        ...
        GLOBAL_LOG_INFO() << "expected: " << env().print(expected_result.value) << ", actual: " << env().print(ent.id());
...
annium_type calculate_type(semantic::logic_tree_node_t & node)
{
    annium_type res = node.true_branch ? calculate_type(*node.true_branch) : node.expression_type;
    if (node.false_branch) {
        annium_type fbranchtype = calculate_type(*node.false_branch);
...
expression_visitor::result_type expression_visitor::operator()(logic_and_expression_t& op) const
{
    ...
    annium_type const* exprt = expected_result ? &expected_result.value : nullptr;
    annium_type ut = make_union_type(annium_any_t{}, exprt);
    ...
    annium_type restype = annium_any_t{};
    ...
    //ctx.pop_chain();
    //return rtype;
    ////return make_union_type(annium_particular_bool_t{ false }, &*rtype); // false || rtype
}
...
expression_visitor::result_type expression_visitor::operator()(binary_operator_t<binary_operator_type::LOGIC_AND>, binary_expression_t& op) const
{
    ...
    auto largvis = expected_result
        ? expression_visitor{ ctx, expected_result_t{ annium_type{annium_any_t{}} || expected_result.value, expected_result.location }}
    ...
    annium_bunion_t const* pbur = rtype.as<annium_bunion_t>();
    annium_bunion_t const* pbul = ltype.as<annium_bunion_t>();

    annium_bunion_t res_type{
...
#if 0
    auto largvis = expected_result
        ? expression_visitor{ ctx, expected_result_t{ expected_result.value || annium_any_t{}, expected_result.location }}
        : expression_visitor{ ctx, nullptr };
...
//{
//    auto func_ent = dynamic_pointer_cast<functional_entity>(ctx.env().eregistry().find(ctx.env().make_qname_identifier("'+"sv)));
//    BOOST_ASSERT(func_ent);
//
//    THROW_NOT_IMPLEMENTED_ERROR();
//}
...
//        ? expression_visitor{ ctx, expected_result_t{ expected_result.value || annium_tuple_t{}, expected_result.location } }
//        : expression_visitor{ ctx, nullptr };
//
...
expression_visitor::result_type expression_visitor::operator()(property_expression& pe)  const
{
    ...
    //if (ctx.context_type == annium_tuple_t{}) {
    //    return std::unexpected(make_error<left_not_an_object_error>(pe.name.location, pe.name.value, annium_tuple_t{}));
    //}
...
expression_visitor::result_type expression_visitor::operator()(function_call_t const& proc) const
{
    ...
    //    return make_error<basic_general_error>(aid.location, "repeated argument"sv, ctx.env().qnregistry().resolve(aid.value));
    //}
    ...
    ct_expression_visitor vis{ ctx, annotated_entity_identifier{ env().get(builtin_eid::qname), proc.location() } };
    auto qn_ent_id = apply_visitor(vis, proc.fn_object);
    if (!qn_ent_id) return std::unexpected(std::move(qn_ent_id.error()));
    qname_identifier_entity qname_ent = static_cast<qname_identifier_entity const&>(env().eregistry_get(*qn_ent_id));
    ...
    //}
    ////GLOBAL_LOG_INFO() << "function call: " << env().print(fnvar->name.value);
...
expression_visitor::result_type expression_visitor::operator()(opt_named_syntax_expression_list_t const& nel) const
{
    ...
    //}
        ...
        //entity_signature isg{ env().get_tuple_qname_identifier() };
...
expression_visitor::expression_visitor(fn_compiler_context& c) noexcept
    : semantic::managed_expression_list{ c.env() }
...
expression_visitor::result_type expression_visitor::handle(base_expression_visitor::result_type&& res) const
{
    ...
    if (ser.is_const_result) {
        ...
        entity const& e = get_entity(ctx.env(), ser.value());
