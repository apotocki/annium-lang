...
class entity_task_id : public sonia::lang::compiler_task_id
{
...
class qname_task_id : public sonia::lang::compiler_task_id
{
...
public:
    ...
    bool equal(sonia::lang::compiler_task_id const& tid) const noexcept override
    {
...
public:
    ...
    using lookup_entity_result_t = std::variant<entity_identifier, local_variable, functional_variable>;
    lookup_entity_result_t lookup_entity(identifier) const;
    lookup_entity_result_t lookup_entity(qname_view) const;
...
#if 0
        if (name.is_absolute()) {
            return environment_.eregistry().find(name);
        }
        qname checkns = ns_;
        qname_view name_qn = environment_.qnregistry().resolve(name);
        for (size_t sz = checkns.parts().size();;) {
            checkns.append(name_qn);
            shared_ptr<entity> e = environment_.eregistry().find(environment_.qnregistry().resolve(checkns));
            if (e || !sz) return e;
            --sz;
            checkns.truncate(sz);
        }
...
#if 0
        if (name.is_absolute()) {
            shared_ptr<entity> e = environment_.eregistry().find(name);
        }
...
#endif
    //semantic::expression_type build_expression(annium_generic_type const& result_type, expression_t const& e);
    ...
    //{
    //    identifier nid = environment_.slregistry().resolve(name);
    //    qname var_qname = ns() / nid;
    //    qname_identifier qname_id = environment_.qnregistry().resolve(var_qname);
    //    functional& varfn = environment_.fregistry().resolve(qname_id);
    ...
    //    identifier argid = environment_.slregistry().resolve(string_view{ tailored_param_name.data(), tailored_param_name.size() });
    ...
    variable_entity& new_captured_variable(identifier name, entity_identifier t, variable_entity& caption)
    {
        ...
        auto ve = sonia::make_shared<variable_entity>(env().qnregistry().resolve(var_qname), std::move(t), variable_entity::kind::LOCAL);
        ve->set_weak(caption.is_weak());
        environment_.eregistry_insert(ve);
...
#endif
    /*
    variable_entity const& new_variable(identifier name, annium_generic_type t, bool is_const)
    {
