...
class function_invoker : public sonia::invocation::functor_object
{
    ...
    sonia::invocation::invocable* penv_;
    sonia::smart_blob capture_;
    small_string name_; // for describing purposes only, it's @XXX for lambdas

public:
    explicit function_invoker(size_t address, bool is_vidx, size_t paramcnt, bool is_void, sonia::smart_blob capture,
        environment& e, sonia::invocation::invocable* pev, small_string name)
            : address_{ address }
    ...
    sonia::smart_blob operator()(std::span<const blob_result> args) override
    {
        ...
        environment_.bvm().run(ctx, procaddress);
        if (is_void_) {
...
void vm::context::construct_function()
{
    ...
    smart_blob res{ object_blob_result<function_invoker>(address, is_vidx, paramcount, is_void, std::move(capture), environment_, penv_, std::move(name)) };
    stack_pop(5);
