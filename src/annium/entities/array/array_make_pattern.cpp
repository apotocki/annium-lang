...
std::expected<functional_match_descriptor_ptr, error_storage> array_make_pattern::try_match(fn_compiler_context& ctx, prepared_call const& call, expected_result_t const& exp) const
{
    ...
    if (of_res) {
        ...
        pmd->signature.emplace_back(e.get(builtin_id::of), of_arg.value_or_type, true);
    ...
    for (size_t arg_num = pmd->has_explicit_type ? 1 : 0;;++arg_num) {
        ...
        if (ser.is_const_result) {
            elem_type = get_entity(e, ser.value()).get_type();
        } else {
            elem_type = ser.type();
            pmd->all_const = false;
        }
    ...
    if (!pmd->has_explicit_type) {
        ...
        pmd->element_type = e.make_union_type_entity(stable_element_types_set).id;
    }
...
std::expected<syntax_expression_result_t, error_storage> array_make_pattern::apply(fn_compiler_context& ctx, semantic::expression_list_t& el, functional_match_descriptor& md) const
{
    ...
    entity_identifier array_type_id = e.make_array_type_entity(amd.element_type, mspan.size()).id;
    entity_signature data_sig{ e.get(builtin_qnid::data), array_type_id };
    ...
    for (auto& [_, er, loc] : mspan) {
        ...
        if (er.is_const_result) {
            ...
            cast_call.emplace_back(make_indirect_value(e, el, std::move(er), loc));
        }
        auto match = ctx.find(builtin_qnid::implicit_cast, cast_call, el,
    ...
    if (amd.all_const) {
        result.value_or_type = e.make_basic_signatured_entity(std::move(data_sig)).id;
    } else {
        size_t element_count = mspan.size();
        if (element_count > 1) {
            e.push_back_expression(el, result.expressions, semantic::push_value{ smart_blob{ ui64_blob_result(element_count) } });
