...
public:
    ...
    inline ellipsis_match_descriptor(qname_identifier fname, EntityT const* ent, resource_location const& loc) noexcept
...
std::expected<syntax_expression_result, error_storage> ellipsis_pattern::apply(fn_compiler_context& ctx, semantic::expression_list_t& el, functional_match_descriptor& md) const
{
    ...
    return apply_visitor(make_functional_visitor<result_t>([&ctx, &nsmd, &el](auto const* pe) -> result_t {
        ...
        if constexpr (std::is_same_v<decltype(pe), identifier_entity const*>) {
            ...
            entity_signature sig{ e.get(builtin_qnid::tuple), e.get(builtin_eid::typename_) };
            for (auto const& field : bse->signature()->fields()) {
                entity const& metaobject_ent = get_entity(e, field.entity_id());
            ...
            if (argcount > 1) {
                e.push_back_expression(el, l, semantic::push_value{ smart_blob{ ui64_blob_result(argcount) } });
