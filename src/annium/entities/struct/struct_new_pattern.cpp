...
std::expected<functional_match_descriptor_ptr, error_storage> struct_new_pattern::try_match(fn_compiler_context& ctx, prepared_call const& call, expected_result_t const&) const
{
    ...
    auto type_arg_descr = call_session.get_named_argument(tpid, expected_result_t{ .modifier = value_modifier_t::constexpr_value });
    if (!type_arg_descr) return std::unexpected(std::move(type_arg_descr.error()));

    resource_location const& typeargloc = type_arg_descr->expression->location;
    syntax_expression_result& type_arg_er = type_arg_descr->result;
...
#if 0
    functional const& fn = e.fregistry().resolve(e.get(builtin_qnid::set));
...
std::expected<syntax_expression_result, error_storage> struct_new_pattern::apply(fn_compiler_context& ctx, semantic::expression_list_t&, functional_match_descriptor& md) const
{
    ...
    entity const& tplent = e.eregistry_get(*uteid);
    entity_signature const* tplsig = tplent.signature();
    BOOST_ASSERT(tplsig && tplsig->name() == e.get(builtin_qnid::tuple));
    ...
    if (argcount > 1) {
        e.push_back_expression(args, semantic::push_value{ mp::integer{ argcount } });
        e.push_back_expression(args, semantic::invoke_function(e.get(builtin_eid::arrayify)));
    }
...
std::ostream& struct_new_pattern::print(environment const&, std::ostream& s) const
{
