...
std::expected<functional_match_descriptor_ptr, error_storage> fixed_array_elements_implicit_cast_pattern::try_match(fn_compiler_context& ctx, prepared_call const& call, expected_result_t const& exp) const
{
    ...
    for (auto const& arg : call.args) {
        ...
        auto res = base_expression_visitor::visit(ctx, call.expressions, argexpr);
        if (!res) return std::unexpected(std::move(res.error()));
        auto& ser = res->first;
        if (ser.is_const_result) {
            if (pmd) {
                return std::unexpected(make_error<basic_general_error>(argexpr.location, "argument mismatch"sv, argexpr));
            }
            ...
            if (!ptypesig || ptypesig->name != e.get(builtin_qnid::array)) {
                return std::unexpected(make_error<type_mismatch_error>(argexpr.location, ent.id, "an array"sv));
            }
            ...
            if (arg_arr_sz != arrsz) {
                return std::unexpected(make_error<value_mismatch_error>(argexpr.location, ent.id, "an array of the same size"sv));
            }
            ...
            for (size_t i = 0; i < arrsz; ++i) {
                ...
                syntax_expression tmpexpr{ argexpr.location, pargeld->entity_id() };
                auto res = base_expression_visitor::visit(ctx, call.expressions, expected_result_t{ .type = arr_element_type_eid, .location = exp.location, .modifier = value_modifier_t::constexpr_value }, tmpexpr);
                if (!res) return std::unexpected(std::move(res.error()));
