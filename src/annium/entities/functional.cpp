...
#include "annium/errors/function_call_match_error.hpp"
#include "annium/errors/pattern_match_error.hpp"
...
entity_signature functional_match_descriptor::build_signature(environment & e, qname_identifier name)
{
...
//            entity const& e = ctx.env().eregistry_get(eid_or_el);
//            //if (e.get_type() == ctx.env().get(builtin_eid::typename_)) {
...
//            return v;
//        } else {
//            if (ctx.context_type == ctx.env().get(builtin_eid::void_)) {
