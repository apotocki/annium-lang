...
std::expected<functional_match_descriptor_ptr, error_storage> to_callable_implicit_cast_pattern::try_match(fn_compiler_context& ctx, prepared_call const& call, expected_result_t const& exp) const
{
    ...
    auto pmd = make_shared<to_callable_implicit_cast_match_descriptor>(call, *psig, exp.modifier);
    ...
    if (!fne.is_built()) {
        ...
        if (!arg_er.is_const_result) {
            local_variable arglv{ .type = arg_er.value_or_type, .varid = env.new_variable_identifier(), .is_weak = false };
            fne.context().push_scope_variable(arglv);
            env.push_back_expression(call.expressions, arg_er_ref.expressions, semantic::push_local_variable::create(arglv));
        }

        for (field_descriptor const& fd : args_span) {
            if (fd.is_const()) continue; // skip const fields for now
            local_variable arglv{ .type = fd.entity_id(), .varid = env.new_variable_identifier(), .is_weak = false };
            fne.context().push_scope_variable(arglv);
            if (auto field_name = fd.name(); field_name) {
                ftor_args.emplace_back(fd.name(), syntax_expression{ arg_loc, local_variable_expression::from_var(arglv) });
            } else {
                ftor_args.emplace_back(syntax_expression{ arg_loc, local_variable_expression::from_var(arglv) });
            }
        }

        expected_result_t callable_expected_result {
            .type = get_entity_type(env, result_fd),
        ...
        if (!psig->fields().back().is_const()) {
            rst.emplace(return_statement{ .expression = make_indirect_value(env, call.expressions, std::move(res->first), call.location), .location = call.location });
        } else {
            rst.emplace(expression_statement{ .expression = make_indirect_expression(env, call.expressions, std::move(res->first), call.location) });
        }
        fne.set_body(span<const statement>{ &*rst, 1 });
        fne.set_need_framepointer(true);
        fne.set_provision(false);

        auto err = fne.build();
        if (err) {
            return std::unexpected(append_cause(
                make_error<basic_general_error>(call.location, "failed to build proxy function for implicit callable cast"sv),
                std::move(err)
            ));
        }
        pmd->pfne = &fne;
    ...
    std::vector<local_variable> arg_vars;
    for (field_descriptor const& fd : args_span) {
        if (fd.is_const()) continue; // skip const fields for now
        arg_vars.emplace_back(local_variable{ .type = fd.entity_id(), .varid = env.new_variable_identifier(), .is_weak = false });
        intptr_t offset = (intptr_t)ftor_args.size() - callable_arg_count;
        if (auto field_name = fd.name(); field_name) {
            ftor_args.emplace_back(fd.name(), syntax_expression{ arg_loc, local_variable_expression::from_var(arg_vars.back()) });
        } else {
            ftor_args.emplace_back(syntax_expression{ arg_loc, local_variable_expression::from_var(arg_vars.back()) });
        }
    }
...
shared_ptr<internal_function_entity> to_callable_implicit_cast_pattern::build(fn_compiler_context& ctx, semantic::expression_list_t& el, to_callable_implicit_cast_match_descriptor& md) const
{
    ...
    pife->set_need_framepointer(true);
...
std::expected<syntax_expression_result, error_storage> to_callable_implicit_cast_pattern::apply(fn_compiler_context& ctx, semantic::expression_list_t& el, functional_match_descriptor& md) const
{
    ...
    if (ser.is_const_result && can_be_constexpr(tmd.result_modifier)) {
        return syntax_expression_result{
            .value_or_type = tmd.pfne->id,
            .is_const_result = true
        };
    }
    
    intptr_t rt_id = env.retrieve_function_rt_identifier(*tmd.pfne);

    syntax_expression_result result{
        .value_or_type = tmd.pfne->get_type(),
