...
void type_entity2::treat(fn_compiler_context& ctx)
{
    ...
    if (handled_) return;
    if (handling_) {
        throw exception(ctx.env().print(basic_general_error{location(), "recursive dependency"sv, name()}));
    }
    handling_ = true;
    signatures.emplace_back();
    auto& sig = signatures.back();

    sig.fn_type.result = annium_object_t{ this };
    ...
    for (auto const& base : direct_bases) {
        ...
        if (!e) [[unlikely]] {
            throw exception(ctx.env().print(undeclared_identifier_error{base.location, base.value}));
        }
        shared_ptr<type_entity> type_ent = dynamic_pointer_cast<type_entity>(e);
        if (!type_ent) [[unlikely]] {
            throw exception(ctx.env().print(basic_general_error{base.location, "not a type identifier"sv, base.value}));
        }
        type_ent->treat(ctx);
...
bool type_entity2::try_cast(fn_compiler_context& ctx, annium_type const& rtype) const
{
    THROW_NOT_IMPLEMENTED_ERROR("type_entity::try_cast");
#if 0
    annium_object_t const* pot = get<annium_object_t>(&rtype);
...
std::expected<function_signature const*, error_storage> type_entity2::find(fn_compiler_context& ctx, pure_call_t& call) const
{
    ...
    ctx.append_expression(semantic::push_value{ ctx.env().as_string(name()) }); // type
    ctx.append_expression(ctx.env().get_builtin_function(environment::builtin_fn::extern_object_create));
    ...
    for (auto & narg : call.named_args) {
        ...
        ctx.context_type = annium_object_t{ this };
        ...
        if (argparts.size() > 1) {
            ...
            annium_type proptype = getter->result_type();
        ...
        if (it == sig.named_parameters().end() || qname{std::get<0>(*it).value} != argname.value) {
            annotated_qname_identifier aqnid{ ctx.env().qnregistry().resolve(argname.value), argname.location };
        ...
        ctx.append_expression(semantic::push_value{ ctx.env().as_string(argname.value) });
#endif
    }
    //ctx.append_expression(semantic::push_value{ decimal{ call.named_args.size() } });
    //ctx.append_expression(semantic::push_value{ ctx.env().as_string(name()) });
    ...
    if (it == funcions_.end()) {
        ...
        for (identifier argid : ids) {
            fn->body.emplace_back(semantic::push_value{ ctx.env().as_string() });
        }

        ctx.env().put_function(fn);
    ...
    result.emplace_back(std::move(args_exprs), annium_object_t{ name() });
    */
#endif
}
...
std::expected<function_entity const*, error_storage> type_entity2::find_field_getter(fn_compiler_context& ctx, annotated_identifier const& f) const
{
...
#if 0
    qname fnname = ctx.env().qnregistry().resolve(name_) / f.value / ctx.env().slregistry().resolve("g"sv);
    qname_identifier fnnameid = ctx.env().qnregistry().resolve(fnname);
    if (auto pfn = dynamic_pointer_cast<function_entity>(ctx.env().eregistry().find(fnnameid)); pfn) {
        return pfn.get();
    }
    function_signature const& sig = signatures.back();
    ...
    if (it == sig.named_parameters().end() || std::get<0>(*it) != f) {
        annotated_qname_identifier aqnid{ ctx.env().qnregistry().resolve(qname{f.value}), f.location };
        return std::unexpected(make_error<parameter_not_found_error>(name(), aqnid));
    }

    function_signature fn_getter_sig;
    fn_getter_sig.position_parameters().emplace_back(annium_object_t{ this });
    fn_getter_sig.fn_type.result = std::get<1>(*it);
    fn_getter_sig.normilize(ctx);
    fn_getter_sig.build_mangled_id(ctx.env());
    auto fnent = sonia::make_shared<function_entity>(fnnameid, std::move(fn_getter_sig));
    fnent->set_inline();
    fnent->body.emplace_back(semantic::push_value{ ctx.env().as_string(f.value) });
    fnent->body.emplace_back(ctx.env().get_builtin_function(environment::builtin_fn::extern_object_get_property));
...
std::expected<function_entity const*, error_storage> type_entity2::find_field_setter(fn_compiler_context& ctx, annotated_identifier const& f) const
{
...
#if 0
    qname fnname = ctx.env().qnregistry().resolve(name_) / f.value / ctx.env().slregistry().resolve("s"sv);
    qname_identifier fnnameid = ctx.env().qnregistry().resolve(fnname);
    if (auto pfn = dynamic_pointer_cast<function_entity>(ctx.env().eregistry().find(fnnameid)); pfn) {
        return pfn.get();
    }
    ...
    fn_setter_sig.position_parameters().emplace_back(annium_object_t{this});
    ...
    //fn_setter_sig.build_mangled_id(ctx.env());
    auto fnent = sonia::make_shared<function_entity>(fnnameid, std::move(fn_setter_sig));
    fnent->set_inline();
    fnent->body.emplace_back(semantic::push_value{ ctx.env().as_string(f.value) });
    fnent->body.emplace_back(ctx.env().get_builtin_function(environment::builtin_fn::extern_object_set_property));
