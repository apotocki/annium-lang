...
std::expected<functional_match_descriptor_ptr, error_storage> metaobject_typeof_pattern::try_match(fn_compiler_context& ctx, prepared_call const& call, expected_result_t const&) const
{
    ...
    identifier objid = e.get(builtin_id::object);
    identifier propid = e.get(builtin_id::property);
    ...
    ct_expression_visitor eobjvis{ ctx, call.expressions, expected_result_t{ e.get(builtin_eid::metaobject), true, object_arg->name()->location } };
    auto obj = apply_visitor(eobjvis, object_arg->value());
    if (!obj) return std::unexpected(std::move(obj.error()));

    ct_expression_visitor epropvis{ ctx, call.expressions, expected_result_t{ e.get(builtin_eid::identifier), true, property_arg->name()->location } };
...
std::expected<syntax_expression_result_t, error_storage> metaobject_typeof_pattern::apply(fn_compiler_context& ctx, semantic::expression_list_t& el, functional_match_descriptor& md) const
{
    ...
    auto& obj = md.get_match_result(e.get(builtin_id::object)).results.front();
    auto& prop = md.get_match_result(e.get(builtin_id::property)).results.front();
    ...
    BOOST_ASSERT(objsignature->name == e.get(builtin_qnid::metaobject));
    
    identifier_entity const& prop_ent = static_cast<identifier_entity const&>(get_entity(e, prop.value()));
