...
#include "annium/errors/no_position_argument_error.hpp"
...
error_storage prepared_call::prepare()
{
    ...
    for (auto it = args.begin(), eit = args.end(); it != eit; ++it) {
        ...
        entity const& type_ent = get_entity(e, type_eid);
        ...
        for (field_descriptor const& fd : signature->fields()) {
            ...
            if (fd.is_const()) {
                ...
                if (subgropname) {
                    entity const& elem_type_ent = get_entity(e, elem_ser.type());
                    entity_signature const* elem_signature = elem_type_ent.signature();
                    BOOST_ASSERT(elem_signature && elem_signature->name == e.get(builtin_qnid::tuple) && elem_signature->field_count() == 2);
                ...
                append_arg(fd_groupname, make_indirect_value(e, expressions, std::move(elem_ser), arg_expr_loc));
            }
            ++argpos;
        }
    }
