...
#include "numeric_literal_minus_pattern.hpp"
...
class numeric_literal_minus_match_descriptor : public functional_match_descriptor
{
public:
    using functional_match_descriptor::functional_match_descriptor;

    numeric_literal_minus_match_descriptor(prepared_call const& call, generic_literal_entity const* lhs_arg, generic_literal_entity const* rhs_arg) noexcept
...
// is responsible for keeping lhs/rhs order when actually emitting the subtraction.
builtin_eid widen_literal_for_runtime_peer(generic_literal_entity const& lit, builtin_eid lit_type, builtin_eid runtime_type)
{
    ...
    case builtin_eid::decimal: {
        numetron::decimal_view v = lit.value().as<numetron::decimal_view>();
        return widen_for_literal_fit(runtime_type, v, lit_type);
    }
    ...
    default:
        THROW_NOT_IMPLEMENTED_ERROR("numeric_literal_minus_pattern: unsupported literal type"sv);
    }
}
...
numeric_literal_minus_pattern::try_match(fn_compiler_context& ctx, prepared_call const& call, expected_result_t const&) const
{
    ...
    // Arity gate: a single-argument call (unary `-x`) is left for numeric_literal_unary_minus_pattern,
    // which shares this qname -- failing here (no second positional argument) falls through to it.
    ...
    auto pmd = sonia::make_shared<numeric_literal_minus_match_descriptor>(call, lhs_literal, rhs_literal);
    ...
    if (lhs_literal && rhs_literal) {
        ...
        smart_blob diff = subtract_numeric(lhs_literal->value(), rhs_literal->value(), result_type);
        pmd->signature.result.emplace(e.make_generic_entity(std::move(diff), e.get(result_type)).id, true);
...
numeric_literal_minus_pattern::apply(fn_compiler_context& ctx, semantic::expression_list_t& el, functional_match_descriptor& md) const
{
    auto& nmd = static_cast<numeric_literal_minus_match_descriptor&>(md);
    ...
    if (nmd.lhs_arg && nmd.rhs_arg) {
        // Both operands were constexpr; try_match already folded the difference into signature.result.
    ...
    // lhs is always appended before rhs -- annium_operator_minus_numeric reads
    // stack_back(1)/stack_back() as lhs/rhs in exactly that order, so operand order (unlike
    // addition) must be preserved regardless of which side, if either, was a literal.
    ...
    e.push_back_expression(el, result.expressions, semantic::invoke_function(e.get(builtin_eid::subtract_numeric)));
