...
#include "numeric_literal_bit_and_pattern.hpp"
...
class numeric_literal_bit_and_match_descriptor : public functional_match_descriptor
{
public:
    using functional_match_descriptor::functional_match_descriptor;

    numeric_literal_bit_and_match_descriptor(prepared_call const& call, generic_literal_entity const* lhs_arg, generic_literal_entity const* rhs_arg) noexcept
...
// Reads `lit`'s value (of type `lit_type`, guaranteed integral-kind by try_match's
// is_integral_kind_eid check) and finds the minimal widening of `runtime_type` it fits into.
// Unlike numeric_literal_plus_pattern's widen_literal_for_runtime_peer, the candidate set here is
// restricted to integral-kind types -- a bitwise-AND result must never land on a float, even if
// some float candidate would otherwise be the first safe common supertype.
builtin_eid widen_literal_for_runtime_peer(generic_literal_entity const& lit, builtin_eid lit_type, builtin_eid runtime_type)
{
    ...
    THROW_INTERNAL_ERROR("numeric_literal_bit_and_pattern: no common safe integer type"sv);
}

} // anonymous namespace

std::expected<functional_match_descriptor_ptr, error_storage>
numeric_literal_bit_and_pattern::try_match(fn_compiler_context& ctx, prepared_call const& call, expected_result_t const&) const
{
    ...
    auto pmd = sonia::make_shared<numeric_literal_bit_and_match_descriptor>(call, lhs_literal, rhs_literal);
    ...
    if (lhs_literal && rhs_literal) {
        ...
        smart_blob conj = bit_and_numeric(lhs_literal->value(), rhs_literal->value(), result_type);
        pmd->signature.result.emplace(e.make_generic_entity(std::move(conj), e.get(result_type)).id, true);
...
numeric_literal_bit_and_pattern::apply(fn_compiler_context& ctx, semantic::expression_list_t& el, functional_match_descriptor& md) const
{
    auto& nmd = static_cast<numeric_literal_bit_and_match_descriptor&>(md);
    ...
    e.push_back_expression(el, result.expressions, semantic::invoke_function(e.get(builtin_eid::bitand_numeric)));
