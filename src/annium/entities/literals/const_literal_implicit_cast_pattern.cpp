...
const_literal_implicit_cast_pattern::try_match(fn_compiler_context& ctx, prepared_call const& call, expected_result_t const& exp) const
{
    ...
    if (exp.type && exp.type != e.get(builtin_eid::any) &&
        exp.type != e.get(builtin_eid::integer) && exp.type != ctx.env().get(builtin_eid::decimal) &&
        exp.type != e.get(builtin_eid::string) &&
    ...
    entity const& src_arg_entity = get_entity(e, src_arg_er.value());
    
    // string to string check
    if (exp.type == e.get(builtin_eid::string)) {
        if (src_arg_entity.get_type() != e.get(builtin_eid::string)) {
            return std::unexpected(make_error<type_mismatch_error>(get_start_location(*get<0>(self_expr)), src_arg_entity.get_type(), exp.type));
        }
    } else if (src_arg_entity.get_type() == e.get(builtin_eid::string) && exp.type != e.get(builtin_eid::any)) {
        return std::unexpected(make_error<type_mismatch_error>(get_start_location(*get<0>(self_expr)), src_arg_entity.get_type(), exp.type));
    }
            ...
            switch (static_cast<builtin_eid>(target_type.value)) {
                ...
                case builtin_eid::i8: {
                    ...
                    if (nmd.is_constexpr_result) {
                        ...
                        auto const& result_entity = e.make_generic_entity(std::move(blob), target_type);
        ...
        if constexpr (std::is_same_v<integer_literal_entity, std::decay_t<decltype(v)>>) {
            ...
            case builtin_eid::string:
                if (nmd.is_constexpr_result) {
                    ...
                    e.push_back_expression(el, src.expressions, semantic::push_value{ smart_blob(v.value()) });
                }
                break;
            default:
