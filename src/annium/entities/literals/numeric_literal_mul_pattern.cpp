...
#include "numeric_literal_mul_pattern.hpp"
...
class numeric_literal_mul_match_descriptor : public functional_match_descriptor
{
public:
    using functional_match_descriptor::functional_match_descriptor;

    numeric_literal_mul_match_descriptor(prepared_call const& call, generic_literal_entity const* lhs_arg, generic_literal_entity const* rhs_arg) noexcept
...
// fits into -- see widen_for_literal_fit in numeric_promotion.hpp.
builtin_eid widen_literal_for_runtime_peer(generic_literal_entity const& lit, builtin_eid lit_type, builtin_eid runtime_type)
{
    ...
    default:
        THROW_NOT_IMPLEMENTED_ERROR("numeric_literal_mul_pattern: unsupported literal type"sv);
    }
}
...
numeric_literal_mul_pattern::try_match(fn_compiler_context& ctx, prepared_call const& call, expected_result_t const&) const
{
    ...
    auto pmd = sonia::make_shared<numeric_literal_mul_match_descriptor>(call, lhs_literal, rhs_literal);
    ...
    if (lhs_literal && rhs_literal) {
        ...
        smart_blob prod = multiply_numeric(lhs_literal->value(), rhs_literal->value(), result_type);
        pmd->signature.result.emplace(e.make_generic_entity(std::move(prod), e.get(result_type)).id, true);
...
numeric_literal_mul_pattern::apply(fn_compiler_context& ctx, semantic::expression_list_t& el, functional_match_descriptor& md) const
{
    auto& nmd = static_cast<numeric_literal_mul_match_descriptor&>(md);
    ...
    if (nmd.lhs_arg && nmd.rhs_arg) {
        // Both operands were constexpr; try_match already folded the product into signature.result.
    ...
    e.push_back_expression(el, result.expressions, semantic::invoke_function(e.get(builtin_eid::multiply_numeric)));
