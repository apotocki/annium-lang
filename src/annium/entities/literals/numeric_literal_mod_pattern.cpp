...
#include "numeric_literal_mod_pattern.hpp"
...
class numeric_literal_mod_match_descriptor : public functional_match_descriptor
{
public:
    using functional_match_descriptor::functional_match_descriptor;

    numeric_literal_mod_match_descriptor(prepared_call const& call, generic_literal_entity const* lhs_arg, generic_literal_entity const* rhs_arg) noexcept
...
// Same as numeric_literal_div_pattern's helper of the same name -- reads `lit`'s value (of type
// `lit_type`) and finds the minimal widening of `runtime_type` it fits into.
builtin_eid widen_literal_for_runtime_peer(generic_literal_entity const& lit, builtin_eid lit_type, builtin_eid runtime_type)
{
    ...
    default:
        THROW_NOT_IMPLEMENTED_ERROR("numeric_literal_mod_pattern: unsupported literal type"sv);
    }
}

// Same as numeric_literal_div_pattern's helper of the same name.
...
numeric_literal_mod_pattern::try_match(fn_compiler_context& ctx, prepared_call const& call, expected_result_t const&) const
{
    ...
    auto pmd = sonia::make_shared<numeric_literal_mod_match_descriptor>(call, lhs_literal, rhs_literal);
    pmd->append_arg(lhs_er, lhs_loc);
    pmd->append_arg(rhs_er, rhs_loc);

    if (lhs_literal && rhs_literal) {
        // Both constexpr: fold entirely at compile time. Unlike division, decimal has no
        // constexpr escape hatch here -- there's no design yet for what a decimal modulo should
        // mean (see FUTURE_WORK.md), so a join landing on decimal is rejected outright, even
        // though both operand values are already known.
        builtin_eid result_type = strongest_numeric_type(lhs_type, rhs_type);
        if (result_type == builtin_eid::decimal) {
            return std::unexpected(make_error<type_mismatch_error>(lhs_loc, lhs_type_id, "a non-decimal numeric type (modulo is not defined for decimal)"sv));
        }
        smart_blob rem = modulo_numeric(lhs_literal->value(), rhs_literal->value(), result_type);
        pmd->signature.result.emplace(e.make_generic_entity(std::move(rem), e.get(result_type)).id, true);
...
numeric_literal_mod_pattern::apply(fn_compiler_context& ctx, semantic::expression_list_t& el, functional_match_descriptor& md) const
{
    auto& nmd = static_cast<numeric_literal_mod_match_descriptor&>(md);
    ...
    if (nmd.lhs_arg && nmd.rhs_arg) {
        // Both operands were constexpr; try_match already folded the remainder into signature.result.
    ...
    // lhs is always appended before rhs -- annium_operator_mod_numeric reads
    ...
    e.push_back_expression(el, result.expressions, semantic::invoke_function(e.get(builtin_eid::modulo_numeric)));
