...
// Handles binary subtraction between any two numeric types (bool excluded -- it's a cast source,
// not an arithmetic operand) without requiring an explicit numeric_cast. Coexists in the "minus"
// qname's overload set with numeric_literal_unary_minus_pattern (unary `-x`): arity distinguishes
// them, same as the old single (integer,integer) binary overload it replaces did. The result type
// is the "strongest" of the two operand types: see numeric_promotion.hpp's strongest_numeric_type().
//
// For constexpr values: computed entirely at compile time.
// For runtime values: delegates to the generic __minus_numeric extern (annium_operator_minus_numeric),
...
// Unlike addition, operand order matters -- lhs and rhs are never swapped.
class numeric_literal_minus_pattern : public functional::pattern
{
public:
    numeric_literal_minus_pattern() = default;
